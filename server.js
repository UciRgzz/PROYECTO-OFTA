...
// ==================== CREAR ORDEN MÉDICA DIRECTA DESDE CONSULTA ====================
// Endpoint para guardar orden médica con todos los campos de exploración clínica
app.post('/api/ordenes_medicas_consulta_directa', verificarSesion, async (req, res) => {
  try {
    const depto = getDepartamento(req);
    const {
      consulta_id,
    ...
    console.log('📋 Creando orden médica directa desde consulta:', consulta_id);

    // Validar campos obligatorios
    if (!consulta_id || !expediente_id || !medico || !diagnostico || !lado || !procedimiento) {
      return res.status(400).json({
        error: 'Faltan campos obligatorios: consulta_id, expediente_id, medico, diagnostico, lado, procedimiento'
      });
    }

    // Validar que el lado sea válido
    const ladosValidos = ['OD', 'OI', 'AMBOS', 'NINGUNO'];
    if (!ladosValidos.includes(lado)) {
      return res.status(400).json({
        error: 'El campo "lado" debe ser OD, OI, AMBOS o NINGUNO'
      });
    }
    ...
    // Crear la orden médica completa
    const result = await pool.query(`
      INSERT INTO ordenes_medicas (
        consulta_id,
        expediente_id,
    ...
    res.status(201).json({
      mensaje: 'Orden médica creada exitosamente',
      orden: result.rows[0]
    });

  } catch (err) {
    console.error('❌ Error creando orden médica directa:', err);
    res.status(500).json({
      error: 'Error al crear la orden médica',
      detalle: err.message
    });
...
// ==================== OBTENER ORDEN MÉDICA POR CONSULTA_ID ====================
app.get('/api/ordenes_medicas_por_consulta/:consultaId', verificarSesion, async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        error: 'No se encontró orden médica para esta consulta'
      });
    }
    ...
    console.error('Error obteniendo orden médica por consulta:', err);
    res.status(500).json({ error: err.message });
  }
});
